...
const start = async () => {
  ...
  if (!fs$1.existsSync(indexPath) || !fs$1.existsSync(path.join(process.cwd(), ".npmrc")) || !fs$1.readFileSync(path.join(process.cwd(), ".npmrc"))?.includes("public-hoist-pattern[]=*sqlite3*")) {
    console.log("检查到项目升级到 1.8.0+ 版本，正在初始化项目...");
    execSync$1("npx karin init", {
      stdio: "inherit",
      cwd: process.cwd()
    });
  }
...
const addBuildDependency = (dependencies) => {
  try {
    ...
    console.error(`错误：添加构建依赖失败 - ${error2.message}`);
  }
};
const removeBuildDependency = (dependencies) => {
  try {
    const content = readWorkspaceFile();
    const depList = dependencies.split(" ").filter((dep) => dep.trim() !== "");
    if (depList.length === 0) {
    ...
    depList.forEach((dependency) => {
      ...
      content.onlyBuiltDependencies = content.onlyBuiltDependencies.filter((dep) => dep !== dependency);
    ...
    console.error(`错误：删除构建依赖失败 - ${error2.message}`);
  }
};
const listBuildDependencies = () => {
  try {
    ...
    content.onlyBuiltDependencies.forEach((dep) => {
      console.log(dep);
    });
    console.log("==== 共 " + content.onlyBuiltDependencies.length + " 个依赖 ====");
  } catch (error2) {
    console.error(`错误：列出构建依赖失败 - ${error2.message}`);
  }
};
const buildDep = {
...
- either make a new Command for each call to parse, or stop storing options as properties`);
    ...
    _checkForMissingExecutable(executableFile, executableDir, subcommandName) {
      if (fs$12.existsSync(executableFile)) return;
    ...
    _executeSubCommand(subcommand, args) {
      ...
      function findFile(baseDir, baseName) {
        ...
        const foundExt = sourceExt.find(
          (ext) => fs$12.existsSync(`${localBin}${ext}`)
        );
      ...
      if (this._scriptPath) {
        ...
        try {
          resolvedScriptPath = fs$12.realpathSync(this._scriptPath);
        } catch {
...
program.command("rs").alias("restart").description("重启后台服务").option("-f, --force", "强制重启").action((options) => pm2.restart(options.force));
...
program.command("b").alias("allow-build").description("构建依赖管理 (pnpm v10.x)").argument("<action>", "操作类型: add, rm, ls").argument("[dependency]", "依赖包名称 (add/rm操作需要 多个使用,分割)").action((action, dependency) => {
